...
interface RoomDao {
    ...
    @Query("SELECT * FROM ${Constants.SQLITE_TABLE_CHAT} WHERE uid= :uid AND status IS NOT 'NEW'  ORDER BY timestamp ASC")
    ...
    @Query("SELECT * FROM ${Constants.SQLITE_TABLE_CHAT} WHERE uid = :uid AND status = 'NEW'")
